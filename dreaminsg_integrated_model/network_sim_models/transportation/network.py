from dreaminsg_integrated_model.network_sim_models.transportation.link import Link
from dreaminsg_integrated_model.network_sim_models.transportation.node import Node
from dreaminsg_integrated_model.network_sim_models.transportation.path import Path
from dreaminsg_integrated_model.network_sim_models.transportation.od import OD
import copy
import datetime
import math
   ...
   def FrankWolfeStepSize(self, targetFlows, precision = FRANK_WOLFE_STEPSIZE_PRECISION):
      ...
      while abs(stepSize - prevStepSize) > precision:
          ...
          for i in temp_link:
              ...
              deltaf = deltaf + temp_link[i].cost*(targetFlows[i] - self.link[i].flow)
              if temp_link[i].flow >0:
                  delta = (temp_link[i].alpha*temp_link[i].beta*temp_link[i].freeFlowTime*pow(vcRatio,temp_link[i].beta))/temp_link[i].flow
                  deltafdash = deltafdash + delta*pow(targetFlows[i] - self.link[i].flow, 2)

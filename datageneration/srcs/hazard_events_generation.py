...
class Event:
    ...
    def datageneration(self):
        ...
        micropolis_network.load_networks(
            water_file, power_file, transp_folder, power_sim_type="3ph"
        )
        ...
        try:
            for x in range(
                self.point_of_occurrence_x[0], self.point_of_occurrence_x[1], 100
            ):
                for y in range(
                    self.point_of_occurrence_y[0], self.point_of_occurrence_y[1], 100
                ):
                    for radius_of_imp in self.radius_of_impact:
                        for inten in self.intensity:

                            explosion = hazard.RadialDisruption(
                                point_of_occurrence=(x, y),
                            ...
                            no_of_datapoints = (
                                self.datapoints / 10
                            ) * self.explosion_datapoints
                            print("no_of_datapoints", no_of_datapoints)
                            explosion.set_affected_components(
                                micropolis_network.integrated_graph
                            )
                            ...
                            if count == no_of_datapoints:
                                ...
                                count = 0
                                raise StopIteration
                            file_is_generated = explosion.generate_disruption_file(
                                location=scenario_location,
                                minimum_data=self.minimum_data,
                            )
        ...
        try:
            for buff_imp in range(self.buffer_impact[0], self.buffer_impact[1], 5):
                for inten in self.intensity:
                    ...
                    micropolis_flood_extents = [
                        (self.point_of_occurrence_x[0], self.point_of_occurrence_y[0]),
                        (self.point_of_occurrence_x[1], self.point_of_occurrence_y[1]),
                    ]
                    flood_track = flood.generate_random_track(
                        micropolis_flood_extents, shape="line"
                    )
                    flood.set_hazard_tracks_from_linestring(flood_track)
                    flood.set_affected_components(
                        micropolis_network.integrated_graph, plot_components=True
                    )
                    scenario_location = MAIN_DIR / network_dir / "scenarios"
                    ...
                    if count == no_of_datapoints:
                        ...
                        count = 0
                        raise StopIteration
                    file_is_generated = flood.generate_disruption_file(
                        location=scenario_location, minimum_data=self.minimum_data
                    )
                    if file_is_generated == 1:
                        count = count + 1
                        file_is_generated = 0
        ...
        try:
            for buff_imp in range(self.buffer_impact[0], self.buffer_impact[1], 5):
                for inten in self.intensity:
                    tornado = hazard.TrackDisruption(
                        hazard_tracks=None,
                        buffer_of_impact=buff_imp,
                        time_of_occurrence=6000,
                        intensity=inten,
                    ...
                    micropolis_tornado_extents = [
                        (self.point_of_occurrence_x[0], self.point_of_occurrence_y[0]),
                        (self.point_of_occurrence_x[1], self.point_of_occurrence_y[1]),
                    ]
                    tornado_track = tornado.generate_random_track(
                        micropolis_tornado_extents, shape="line"
                    )
                    tornado.set_hazard_tracks_from_linestring(tornado_track)
                    tornado.set_affected_components(
                        micropolis_network.integrated_graph, plot_components=True
                    )
                    scenario_location = MAIN_DIR / network_dir / "scenarios"
                    ...
                    if count == no_of_datapoints:
                        ...
                        count = 0
                        raise StopIteration
                    file_is_generated = tornado.generate_disruption_file(
                        location=scenario_location, minimum_data=self.minimum_data
                    )
                    if file_is_generated == 1:
                        count = count + 1
        ...
        try:
            for buff_imp in range(self.buffer_impact[0], self.buffer_impact[1], 5):
                for inten in self.intensity:
                    hurricane = hazard.TrackDisruption(
                        hazard_tracks=None,
                        buffer_of_impact=buff_imp,
                        time_of_occurrence=6000,
                        intensity=inten,
                    ...
                    micropolis_hurricane_extents = [
                        (self.point_of_occurrence_x[0], self.point_of_occurrence_y[0]),
                        (self.point_of_occurrence_x[1], self.point_of_occurrence_y[1]),
                    ]
                    hurricane_track = hurricane.generate_random_track(
                        micropolis_hurricane_extents, shape="line"
                    )
                    hurricane.set_hazard_tracks_from_linestring(hurricane_track)
                    no_of_datapoints = (
                        self.datapoints / 10
                    ) * self.hurricane_datapoints
                    hurricane.set_affected_components(
                        micropolis_network.integrated_graph, plot_components=True
                    )
                    scenario_location = MAIN_DIR / network_dir / "scenarios"
                    if count == no_of_datapoints:
                        ...
                        count = 0
                        raise StopIteration
                    file_is_generated = hurricane.generate_disruption_file(
                        location=scenario_location, minimum_data=self.minimum_data
                    )
                    if file_is_generated == 1:
                        count = count + 1

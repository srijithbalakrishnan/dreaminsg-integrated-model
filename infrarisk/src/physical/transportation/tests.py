...
import infrarisk.src.physical.transportation.network as network
...
def check(name, value, target, tolerance):
    if approxEqual(value, target, tolerance):
        ...
        print("\nWrong %s: your value %s, correct value %s" % (name, value, target))
        return False
...
def readFlowsFile(flowsFileName):
    ...
    try:
        ...
        print(
            "\nError running test %s, attempting to continue with remaining tests.  Exception details: "
            % flowsFileName
        )
